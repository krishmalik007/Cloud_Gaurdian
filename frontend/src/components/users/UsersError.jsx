...
export default function UsersError({ error, onRetry }) {
  const message = error?.response?.data?.detail || error?.message || 'Failed to fetch user database';
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center select-none bg-red/5 border border-red/20 rounded-xl w-full">
      ...
      <p className="text-xs text-text-secondary max-w-sm mb-5 leading-relaxed">
        {message}. Note that User administration is restricted to platform Administrators.
      </p>
